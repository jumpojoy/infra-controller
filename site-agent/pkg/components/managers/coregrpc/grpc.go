...
func createGrpcClient() (conn *client.CoreGrpcClient, err error) {
	// Initialize contextual logger
	logger := log.With().Str("Method", "CoreGrpcClient.createGrpcClient").Logger()
	logger.Info().Msg("Core gRPC: Starting gRPC client")
	...
	logger.Info().Interface("GrpcConfig", ManagerAccess.Data.EB.Managers.CoreGrpc.Client.Config).Msg("Initializing gRPC client")
	...
	return ManagerAccess.Data.EB.Managers.CoreGrpc.GetClient(), nil
}
...
// UpdateGrpcClientState - updates core gRPC state
func (coregrpc *API) UpdateGrpcClientState(err error) {
	...
	log.Error().Err(err).Msg("Core gRPC: Failed to send request to gRPC server")
